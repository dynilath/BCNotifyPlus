...
function CompareNumberArray(
  cur: ServerAccountQueryResultItem[],
  prev: ServerAccountQueryResultItem[]
) {
  ...
  static init(mod: ModSDKModAPI) {
    ...
    mod.hookFunction("ServerAccountQueryResult", 0, (args, next) => {
      ...
      if (this.justLoaded) {
        ...
        let { removed, added } = CompareNumberArray(
          arg.Result,
          this.onlineFriendMemory
        );

        removed.forEach((c) => {
          const doNotify = (() => {
            let specV = notifySetting.notifies.spec.find(
              (_) => _.MemberNumber === c.MemberNumber
            ) as NotifyPlusSpecV2 | undefined;
